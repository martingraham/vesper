...
var AdaptedD3 = new function () {
            ...
            if (dx > cutoff) {
                ...
                if (children && (n = children.length)) {
                    ...
                    if (toGrid.length > 0) {
                        ...
                        griddle (coords, nodeId, x, ax, dy, toGrid, cutoff, node.depth);
                    }
                }
            }
        }
    ...
    function griddle (coords, nodeId, x, xspace, yspace, nodes, cutoff, pdepth) {

        // show as many nodes as we reasonably can if they're below the root
        // can use ordering to see 'important' taxa i.e. selected taxa
        if (pdepth == 0) {
            nodes.length = Math.min (nodes.length, Math.floor ((xspace / cutoff) * (xspace / cutoff)));
        }

        if (xspace / Math.sqrt (nodes.length) < cutoff) {
            console.log ("GRIDFAIL", xspace, nodes.length, Math.sqrt (nodes.length), cutoff, pdepth);
            return;
        }
        console.log ("GRIDGOOD", xspace, nodes.length, Math.sqrt (nodes.length), cutoff, pdepth);
