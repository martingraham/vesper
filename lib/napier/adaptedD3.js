...
var AdaptedD3 = new function () {
            ...
            if (dx > cutoff) {
                ...
                if (children && (n = children.length)) {
                    ...
                    if (toGrid.length > 0) {
                        ...
                        griddle (coords, nodeId, x, ax, dy, toGrid, cutoff);
                    }
                }
            }
        }
    ...
    function griddle (coords, nodeId, x, xspace, yspace, nodes, cutoff) {

        if (xspace / Math.sqrt (coords.length) < cutoff) {
            console.log ("GRIDFAIL", x, Math.sqrt (coords.length), cutoff);
            return;
        }
        console.log ("GRIDGOOD", ax, Math.sqrt (coords.length), cutoff);
