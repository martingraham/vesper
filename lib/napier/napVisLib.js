...
MGNapier.NapVisLib = new function () {
    ...
    // fromthese, toThese are entry arrays of {key,value} object
    // byThese is potentiannly an array of such entry arrays
    this.makeGraph2 = function (dataModel, fromThese, toThese, byThese, linkTheseName, andTheseName, byTheseName) {
        ...
        for (n = 0; n < toThese.length; n++) {
            ...
            if (byTheseName === andTheseName) {
                if (!lmap[tEntry.key]) {
                    lmap[tEntry.key] = {from:[], to:[]};
                }
                lmap[tEntry.key].to.push (tEntry.key);
            }
            else {
    ...
    this.mergeFinalArrays = function (finalArrays, fromThese, toThese) {
        if (finalArrays.length === 1) {
            return finalArrays[0];
        }

        var mergedArray = {};
        for (var n = fromThese.length; --n >= 0;) {
            var from = fromThese[n].key;
            for (var m = toThese.length; --m >- 0;) {
                var to = toThese[m].key;
                for (var k = finalArrays.length; --k >= 0;) {
                    var fval = finalArrays[k][from];
                    if (fval) {
                        var tval = fval[to];
                        if (tval !== undefined) {
                            if (!mergedArray[from]) {
                            ...
                            mergedArray[from][to] = (mergedArray[from][to] || 0) + finalArrays[k][from][to];
                        }
                    }
                }
            }
    ...
    this.rangeSlider = function() {
        ...
        var selRange = [scale.range()[0], scale.range()[1]];
        var orient;
        var _update;
        var deltaVar = "dx";
        ...
        function rangeSlider(g) {
            ...
            function duringDrag () {
                _update ();
                _duringDragFunc (selRange, rangeSlider.scale());
            }

            function getDelta () {
                return d3.event[deltaVar];
            }

            var drag1 = d3.behavior.drag()
                .on("drag", function() {
                    selRange[0] = Math.max (scale.range()[0], Math.min (selRange[1], selRange[0] + getDelta()));
                    duringDrag();
                })
            ;
            ...
            var drag2 = d3.behavior.drag()
                .on("drag", function() {
                    selRange[1] = Math.min (scale.range()[1], Math.max (selRange[0], selRange[1] + getDelta()));
                    duringDrag();
                })
            ;
            ...
            var drag3 = d3.behavior.drag()
                .on("drag", function() {
                    ...
                    var dx = (delta > 0
                        ? Math.min (delta, scale.range()[1] - selRange[1])
                        : Math.max (delta, scale.range()[0] - selRange[0])
                    );
                    selRange[0] += dx;
                    selRange[1] += dx;
                    duringDrag();
            ...
            function dragend () {
                _dragEndFunc (selRange, rangeSlider.scale());
            }
            ...
            _update = function () {
                g.select("rect.sliderRange")
                    .attr ("x", scale.range()[0])
                    .attr ("width", scale.range()[scale.range().length - 1] - scale.range()[0])
                ;

                g.select("rect.sliderMax")
                    .attr("x", function() { return selRange[1]; })
                ;
                g.select("rect.sliderMin")
                    .attr("x", function() { return selRange[0] - d3.select(this).attr("width"); })
                ;
                g.select("rect.sliderBar")
                    .attr("x", function() { return selRange[0]; })
                    .attr("width", function() { return selRange[1] - selRange[0]; })
                ;

                g.selectAll (".sliderText")
                    .data ([tooltipTemplates.min(selRange, scale), tooltipTemplates.max(selRange, scale)])
                    .text (function(d) { return d; })
                    // need to do +g... 'cos otherwise gets interpreted as a string
                    .attr ("x", function(d,i) { return i === 0 ? (selRange[0] - (+g.select("rect.sliderMin").attr("width")) - 2) : (selRange[1] + (+g.select("rect.sliderMax").attr("width")) + 2); })
                ;
        ...
        rangeSlider.scale = function(x) {
            ...
            scale = x.copy();
            var sr = scale.range();
            selRange = [sr[0], sr[sr.length - 1]];
            return rangeSlider;
        };
        rangeSlider.orient = function(x) {
            if (!arguments.length) { return orient; }
            orient = x in d3_svg_axisOrients ? x + "" : d3_svg_axisDefaultOrient;
            deltaVar = (orient === "left" || orient === "right") ? "dy" : "dx";
            return rangeSlider;
        };
        ...
        rangeSlider.setRange = function (x) {
            selRange[0] = Math.max (scale.range()[0], x[0]);
            selRange[1] = Math.min (scale.range()[1], x[1]);
            return rangeSlider;
        };
