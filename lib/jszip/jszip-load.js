...
(function () {
   ...
   ZipEntry.prototype = {
       ...
       */
      readLocalPart : function(reader, callback) {
         ...
         //var fileStats = {start: reader.index, cdata: reader.stream, reader: reader}; // Basically a position in the entire zip file
         ...
         if (compression === null) { // no compression found
          ...
          var assignFunc = function(results) {
              self.uncompressedFileData = results;

              if (!self.streamFunction && self.uncompressedFileData.length !== self.uncompressedSize) {
                  throw new Error("Bug : uncompressed data size mismatch");
              }

              if (self.loadOptions.checkCRC32 && JSZip.prototype.crc32(self.uncompressedFileData) !== self.crc32) {
                  throw new Error("Corrupted zip : CRC32 mismatch");
              }
          };

          var self = this;
          if (this.streamFunction && this.streamFunction.callbackQ) {
              this.streamFunction.callbackQ.push (assignFunc);
          }
          //this.uncompressedFileData =
          var results = compression.uncompress (reader, this.streamFunction);
          if (results != undefined) { // i.e. synchronous return of results
                assignFunc (results);
          }
      },
   ...
   ZipEntries.prototype = {
       ...
       getLocalFile : function (fileName) {
           ...
           for(var i = 0; i < this.files.length; i++) {
               ...
               if (file.fileName == fileName) {
                   console.log (file, file.data ? file.data.length : "not unzipped");
                   return file;
               }
           }
           return null;
      ...
      readLocalFiles : function(streamFunction) {
         ...
         var finalCallbacks;
         if (streamFunction && streamFunction.callbackQ) {
             finalCallbacks = streamFunction.callbackQ;
             streamFunction.callbackQ = [];
         }

         var ii = 0, flength = this.files.length, myFiles = this.files;

        function readNextFile () {
            var file = myFiles[ii];
            this.reader.setIndex(file.localHeaderOffset);
            this.checkSignature(JSZip.signature.LOCAL_FILE_HEADER);
            if (ii == flength - 1 && finalCallbacks) {
                streamFunction.callbackQ = finalCallbacks;
            }
            file.streamFunction = streamFunction;
            ...
            function afterwards () {
                ...
                if (streamFunction.callbackQ) {
                    streamFunction.callbackQ.length = 0;
                }
                ii++;
                if (ii < flength) {
                    readNextFile();
            ...
            if (streamFunction && streamFunction.callbackQ) {
                streamFunction.callbackQ.push (afterwards);
            }
            file.readLocalPart(this.reader);
            if (!streamFunction || !streamFunction.callbackQ) {
                afterwards();
   ...
   JSZip.prototype.load = function(data, options) {
      ...
      files = zipEntries.files;
       if (VESPER && VESPER.alerts) {
        alert ("pause for profiling");
      }
