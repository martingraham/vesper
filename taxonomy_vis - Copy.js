...
VESPER.Tree = function(divid) {
    ...
    this.set = function (fields, mmodel) {
        console.log ("set args", arguments, d3.select(divid).node());
	...
	this.go = function () {
        ...
        if (root === undefined) {
            console.log ("no root defined for tree");
            return;
        }
    ...
    function reroot (root) {
        ...
        ;
        console.log ("what alloc reports ",spaceAlloc.size());
        console.log ("root", root);
        var coordSet = spaceAlloc.nodes (root);
        console.log (coordSet.length, coordSet);
        curRoot = root;
